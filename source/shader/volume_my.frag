...
//calculate just for inside volume
bool
...
vec3
get_gradient_original_task(vec3 pos)
{
	float dim_x = volume_dimensions.x / max_bounds.x;
	float dim_y = volume_dimensions.y / max_bounds.y;
	float dim_z = volume_dimensions.z / max_bounds.z;
...
void main()
{
    ...
    while (inside_volume)
    {
	 // get sample
        ...
        opacity_corr = 1 - pow(1-color.a,255*( sampling_distance / sampling_distance_ref));
#else
#endif
...
//Task 32
	...
	if (length(get_gradient_original_task(sampling_pos)) > 0.1) {
	vec4 norm = vec4(normal , 1.0);
